...
export function AdminUserModerationPage({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    kometaApi.users
      ...
      .then(setUser)
      .catch((caughtError) =>
        setError(caughtError instanceof Error ? caughtError.message : t("User failed to load.")),
      )
  ...
  async function blockUser(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(caughtError instanceof Error ? caughtError.message : t("Block failed."));
    }
  }
  ...
  async function unblockUser() {
    try {
      ...
      setError(caughtError instanceof Error ? caughtError.message : t("Unblock failed."));
    }
  }

  if (isLoading) {
    return <LoadingState label={t("Loading user")} />;
  }

  if (!user) {
    return <ErrorState message={error ?? t("User is unavailable.")} />;
  }

  return (
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
      <section>
        <h1 className="font-heading text-3xl font-semibold">{t("User moderation")}</h1>
        ...
        <Card className="mt-5 rounded-lg">
          ...
          <CardContent className="grid gap-2 text-sm">
            ...
            <span>
              {user.rating.toFixed(1)} {t("rating")}
            </span>
          </CardContent>
        </Card>
      </section>
      <aside className="h-fit rounded-lg border p-5">
        <form className="grid gap-4" onSubmit={blockUser}>
          <div className="grid gap-2">
            <Label htmlFor="reason">{t("Block reason")}</Label>
